...
// ─── Shared helpers ───────────────────────────────────────────────────────────
const PROPERTY_TYPES = ['apartment', 'house', 'villa', 'townhouse', 'land'] as const;
...
// isFavorite mỗi 15 item, isHighlighted mỗi 8 item
const makeProperty = (
  id: number,
  title: string,
  province: string,
...
const generateHaiPhong = (): Property[] =>
  Array.from({ length: 100 }, (_, i) => {
...
// ─── MOCK_PROPERTIES = gốc 50 + 200*4 generated ──────────────────────────────
...
export const getPagedProperties = (
  ...
  return { data: filtered.slice(start, end), hasNextPage: end < filtered.length, total: filtered.length };
};